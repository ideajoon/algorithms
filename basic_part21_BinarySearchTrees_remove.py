    ...
    def remove(self, key):
        ...
        if node:
            ...
            if nChildren == 0:
                ...
                # parent.left 또는 parent.right 를 None 으로 하여 leaf node 였던 자식을 트리에서 끊어내어 없앱니다.
                if parent:
            ...
            # When the node has only one child
                ...
                # 만약 parent 가 있으면
                # node 가 왼쪽 자식인지 오른쪽 자식인지 판단하여 위에서 가리킨 자식을 대신 node 의 자리에 넣습니다.
                if parent:
                    if node.key < parent.key:
            ...
            # When the node has both left and right children
                ...
                # parent 는 node 를 가리키고 있고, successor 는 node 의 오른쪽 자식을 가리키고 있으므로
                # successor 로부터 왼쪽 자식의 링크를 반복하여 따라감으로써
                # 순환문이 종료할 때 successor 는 바로 다음 키를 가진 노드를, 그리고 parent 는 그 노드의 부모 노드를 가리키도록 찾아냅니다.
